...
public class SettingsViewModel: ObservableObject {
    ...
    @Published public var hasElevenLabsKey = false
    ...
    private func loadSettings() {
        ...
        hasElevenLabsKey = KeychainHelper.hasAPIKey(for: "elevenlabs")
        if hasElevenLabsKey {
            elevenLabsAPIKey = KeychainHelper.getAPIKey(for: "elevenlabs") ?? ""
        }
    }
    
    public func saveElevenLabsAPIKey() {
        if !elevenLabsAPIKey.isEmpty {
            AppConfiguration.APIKeys.setElevenLabsKey(elevenLabsAPIKey)
            hasElevenLabsKey = true
            
            // Restart audio service with new key
            Task {
                await DIContainer.shared.resetServices()
            }
        }
    }
    
    public func clearElevenLabsAPIKey() {
        KeychainHelper.deleteAPIKey(for: "elevenlabs")
        elevenLabsAPIKey = ""
        hasElevenLabsKey = false
        
        // Restart audio service to fallback to system
        Task {
            await DIContainer.shared.resetServices()
        }
    }
