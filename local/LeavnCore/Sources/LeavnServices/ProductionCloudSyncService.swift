...
public actor ProductionCloudSyncService: SyncServiceProtocol {
    ...
    private func createBookmark(from record: CKRecord) -> Bookmark? {
        guard let verseName = record["verseName"] as? String,
              let chapterNum = record["chapter"] as? NSNumber,
              let verseNum = record["verse"] as? NSNumber,
        ...
        let chapter = chapterNum.intValue
        let verseNumber = verseNum.intValue
    ...
    private func createReadingPlan(from record: CKRecord) -> ReadingPlan? {
        guard let name = record["name"] as? String,
              ...
              let durationNum = record["duration"] as? NSNumber,
              let isActive = record["isActive"] as? Bool,
              let progressNum = record["progress"] as? NSNumber else {
            return nil
        }
        
        let duration = durationNum.intValue
        let progress = progressNum.doubleValue
    ...
    private func createReadingHistory(from record: CKRecord) -> ReadingHistory? {
        ...
        let chapter = chapterNum.intValue
        let startVerse = (record["startVerse"] as? NSNumber)?.intValue
        let endVerse = (record["endVerse"] as? NSNumber)?.intValue
