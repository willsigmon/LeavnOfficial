...
public final class DIContainer: ObservableObject {
    ...
    @Published public private(set) var audioService: AudioServiceProtocol?
    ...
    private func registerFeatureServices() async {
        ...
        let elevenLabsKey = AppConfiguration.APIKeys.elevenLabsKey
        if !elevenLabsKey.isEmpty {
            audioService = ElevenLabsAudioService(
                apiKey: elevenLabsKey,
                cacheService: cacheService,
                lifeSituationsEngine: lifeSituationsEngine
            )
            logger.info("✅ ElevenLabs Audio Service registered.")
        } else {
            // Use system TTS as fallback
            audioService = SystemAudioService()
            logger.info("✅ System Audio Service registered (fallback mode).")
        }
    }
    ...
    public func requireAudioService() -> AudioServiceProtocol {
        guard let service = audioService else { fatalError("AudioService not available") }
        return service
    }
    
    public func requireLifeSituationsEngine() -> LifeSituationsEngineProtocol {
        guard let service = lifeSituationsEngine else { fatalError("LifeSituationsEngine not available") }
        return service
    }
...
private final class DefaultNetworkService: NetworkServiceProtocol {
    ...
    func request<T: Codable>(_ endpoint: APIEndpoint) async throws -> T {
        guard let url = URL(string: endpoint.path) else {
            throw ServiceError.network(.serverError(statusCode: 400, message: "Invalid URL"))
        }
        ...
        // Add User-Agent header to prevent CFNetwork crashes
        request.setValue("Leavn/1.0", forHTTPHeaderField: "User-Agent")
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            throw ServiceError.network(.serverError(statusCode: statusCode, message: "HTTP error"))
        }
